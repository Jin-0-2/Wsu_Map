...
// 게스트 전용 인증 미들웨어 (게스트만 접근 가능)
const guestAuthMiddleware = (req, res, next) => {
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    console.warn(`[GUEST AUTH FAIL] ${req.method} ${req.originalUrl} - Missing or malformed Authorization header`);
  ...
  try {
    if (!process.env.JWT_SECRET) {
      console.error(`[GUEST AUTH ERROR] ${req.method} ${req.originalUrl} - JWT_SECRET not configured`);
    ...
    // 게스트 토큰 여부 확인
    if (!decoded || decoded.isGuest !== true) {
      console.warn(`[GUEST AUTH FAIL] ${req.method} ${req.originalUrl} - Not a guest token`);
      return res.status(403).json({ success: false, message: '게스트 전용 리소스입니다.' });
    }
    ...
    console.info(`[GUEST AUTH OK] ${req.method} ${req.originalUrl} - guestId=${decoded?.id ?? 'unknown'}`);
    next();
  } catch (error) {
    if (error.name === 'TokenExpiredError') {
      console.warn(`[GUEST AUTH FAIL] ${req.method} ${req.originalUrl} - TokenExpiredError`);
      return res.status(419).json({ success: false, message: '토큰이 만료되었습니다. 다시 로그인해주세요.' });
    }
    console.warn(`[GUEST AUTH FAIL] ${req.method} ${req.originalUrl} - Invalid token (${error.name || 'UnknownError'})`);
